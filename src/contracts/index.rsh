...
export const main = Reach.App(() => {
	...
	const rewardPerBlock =
		(UInt256(totalRewardAmount) * BIG_NUMBER) / UInt256(endBlock - beginBlock)
	...
	const getAlgoReward = (n) =>
		muldiv(n, totalAlgoRewardAmount, totalRewardAmount)
	...
	const claimRewards = (p) => {
		...
		enforce(
			claimedReward <= balance(rewardToken) && extraAlgoReward <= balance()
		)
	...
	const [
		...
		.define(() => {
			...
			const getNewRewardPerToken = () => {
				if (totalStaked > 0) {
					const rewardBlocksPassed = UInt256(
						lastBlockWithRewards() - lastUpdateBlock
					)
					const addedReward =
						(rewardPerBlock * rewardBlocksPassed) / UInt256(totalStaked)
			...
			const updateReward = (p) => {
				...
				const rewardToPayNow =
					(UInt256(staked(p)) *
						(rewardPerTokenStoredNew - rewardPerTokenPaid(p))) /
					BIG_NUMBER
		...
		.api_(Api.claimFees, () => {
			...
			return [
				...
				(callback) => {
					...
					const tokenToClaim = min(
						rewardTokenFees + missed,
						balance(rewardToken)
					)
